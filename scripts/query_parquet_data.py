...
@app.command()
def query(
    table_name: str = typer.Argument(..., help="要查询的表名 (例如: stock_daily)"),
        ...
        with duckdb.connect(str(db_path), read_only=True) as con:
            ...
            if symbol:
                try:
                    ...
                    if not symbol_col:
                        logging.error(f"错误：在表 '{table_name}' 中未找到可用于查询股票代码的列。 সন")
                        raise typer.Exit(1)
                    
                    logging.info(f"在表 '{table_name}' 中使用 '{symbol_col}' 列进行查询。 সন")
                    query += f" WHERE {symbol_col} = ?"
            ...
            if date_col_to_sort:
                ...
                query += f" ORDER BY {date_col_to_sort} DESC"

            query += f" LIMIT ?"
            params.append(limit)

            # 执行查询
            logging.info(f"执行查询: {query} | 参数: {params}")
            result_df = con.execute(query, params).fetch_df()

            if result_df.empty:
                logging.warning("查询成功，但未返回任何数据。 সন")
            else:
