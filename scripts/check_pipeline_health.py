...
class PipelineHealthChecker:
    ...
    def __init__(self):
        ...
        self.parquet_base_path = (
            self.project_root / self.config.storage.parquet_base_path
        )
    ...
    def check_consumer_heartbeat(self, queue_name: str, max_inactive_minutes: int = 30):
        ...
        if not log_file.exists():
            self._add_result(
                f"心跳检查 ({queue_name})", "FAIL", f"日志文件 {log_file} 不存在！"
            )
            return
        ...
        if time.time() - last_modified_time > max_inactive_minutes * 60:
            self._add_result(
                f"心跳检查 ({queue_name})",
                "FAIL",
                f"日志文件在过去 {max_inactive_minutes} 分钟内无更新。",
            )
        else:
    ...
    def check_queue_size(self, queue_name: str, max_size: int = 1000):
        ...
        if not db_path or not db_path.exists():
            self._add_result(
                f"队列积压检查 ({queue_name})", "FAIL", f"队列数据库 {db_path} 不存在！"
            )
            return
            ...
            with duckdb.connect(str(db_path)) as con:
                ...
                if count > max_size:
                    self._add_result(
                        f"队列积压检查 ({queue_name})",
                        "FAIL",
                        f"队列积压了 {count} 个任务，超过阈值 {max_size}！",
                    )
                else:
                    self._add_result(
                        f"队列积压检查 ({queue_name})",
    ...
    def check_data_freshness(
        ...
        if not self.metadata_db_path.exists():
            self._add_result(
                "数据新鲜度检查",
                "FAIL",
                f"元数据数据库 {self.metadata_db_path} 不存在！",
            )
            return
            ...
            with duckdb.connect(str(self.metadata_db_path)) as con:
                ...
                if table_name not in tables:
                    self._add_result(
                        "数据新鲜度检查",
                        "FAIL",
                        f"表 '{table_name}' 在元数据数据库中不存在。",
                    )
                    return

                latest_date_str = con.execute(
                    f"SELECT MAX({date_column}) FROM {table_name}"
                ).fetchone()[0]
                if not latest_date_str:
                    self._add_result(
                        "数据新鲜度检查", "PASS", f"表 '{table_name}' 为空，跳过检查。"
                    )
                    return
                ...
                if datetime.now().date() - latest_date > timedelta(days=max_days_stale):
                    self._add_result(
                        "数据新鲜度检查",
                        "FAIL",
                        f"数据最新的日期是 {latest_date}，超过了 {max_days_stale} 天。",
                    )
                else:
                    self._add_result(
                        "数据新鲜度检查",
                        "PASS",
                        f"数据最新的日期是 {latest_date}，在容忍范围内。",
                    )
        except Exception as e:
    ...
    def check_metadata_freshness(self, max_minutes_lag: int = 30):
        ...
        if not self.metadata_db_path.exists() or not self.parquet_base_path.is_dir():
            self._add_result(
                "元数据新鲜度检查", "FAIL", "元数据DB或Parquet目录不存在。"
            )
            return
        ...
        if latest_parquet_mtime == 0:
            self._add_result(
                "元数据新鲜度检查", "PASS", "未找到任何Parquet文件，跳过检查。"
            )
            return

        metadata_mtime = self.metadata_db_path.stat().st_mtime
        if (
            metadata_mtime < latest_parquet_mtime
            and time.time() - metadata_mtime > max_minutes_lag * 60
        ):
            self._add_result(
                "元数据新鲜度检查",
                "FAIL",
                f"元数据DB自 {datetime.fromtimestamp(metadata_mtime)} 后未更新，但有更新的Parquet文件。",
            )
        else:
            self._add_result(
                "元数据新鲜度检查", "PASS", "元数据与Parquet文件同步状态良好。"
            )
    ...
    def run_all_checks(self):
        ...
        logging.info(
            f"检查总结: 总共 {len(self.results)} 项, 成功 {summary['PASS']} 项, 失败 {summary['FAIL']} 项."
        )
        if summary["FAIL"] > 0:
