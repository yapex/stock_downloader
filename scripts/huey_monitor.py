...
# --- 注册所有可能的任务 ---
# 监控器需要知道所有可能在队列中遇到的任务的定义，
# 否则在反序列化任务时会因找不到注册条目而出错。
try:
    from neo.tasks import huey_tasks  # noqa
except ImportError:
    # 如果在测试或独立运行环境中，这可能会失败，但可以安全忽略。
    pass
# -------------------------
...
if __name__ == "__main__":
    ...
    try:
        with Live(
            generate_table(fast_monitor, slow_monitor, start_time),
            screen=True,
