...
class TestConfig:
    ...
    @patch("builtins.open", new_callable=mock_open, read_data=b'[tushare]\ntoken = "test_token"\n[database]\ntype = "duckdb"\npath = "./test.db"\n[api]\ntimeout = 30\nretry_count = 3')
    ...
    def test_config_loading(self, mock_path_open, mock_file):
        ...
        # 重新导入配置模块以触发配置加载
        import importlib
        if 'downloader2.config' in sys.modules:
            importlib.reload(sys.modules['downloader2.config'])
        else:
            import downloader2.config
        
        from downloader2.config import get
    ...
    @patch("builtins.open", new_callable=mock_open, read_data=b'[stocks]\nsymbols = ["000001.SZ", "000002.SZ", "600000.SH"]\n[exchanges]\nlist = ["SZ", "SH", "BJ"]')
    @patch("pathlib.Path.open")
    ...
    @patch("builtins.open", new_callable=mock_open, read_data=b'[app]\nname = "stock_downloader"\ndebug = true\n[nested]\n[nested.deep]\nvalue = 42\n[nested.deep.deeper]\nfinal = "success"')
    @patch("pathlib.Path.open")
