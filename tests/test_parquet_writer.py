...
class TestParquetWriter:
    ...
    def create_stock_basic_data(self) -> pd.DataFrame:
        ...
        """
        return pd.DataFrame({
            'ts_code': ['000001.SZ', '000002.SZ', '600000.SH', '000858.SZ', '002415.SZ'],
            ...
            'list_date': ['19910403', '19910129', '19990810', '19930608', '20100528'],
            'act_name': ['无实际控制人', '深圳市人民政府国有资产监督管理委员会', 
                        '上海国际集团有限公司', '宜宾五粮液集团有限公司', '龚虹嘉'],
            'act_ent_type': ['无', '地方国企', '地方国企', '地方国企', '境内自然人']
        })
    ...
    def test_write_full_replace_creates_correct_structure(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for file_path in parquet_files:
                assert file_path.suffix == ".parquet", f"文件扩展名应该是 .parquet: {file_path}"
                
            # 验证能正确读取生成的文件
            result_data = pd.read_parquet(target_path)
            assert len(result_data) == len(data), f"数据行数应该一致: {len(result_data)} vs {len(data)}"
