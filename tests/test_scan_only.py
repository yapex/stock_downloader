...
class TestFileScan:
    ...
    def test_fast_file_scan(self):
        ...
        assert result["file_count"] == 9, (
            f"_fast_file_scan应该找到9个文件，实际找到{result['file_count']}个"
        )
    ...
    def test_quick_table_check(self):
        ...
        assert result["file_count"] == 9, (
            f"_quick_table_check应该找到9个文件，实际找到{result['file_count']}个"
        )
        assert result["last_modified"] is not None
    ...
    def test_get_file_info(self):
        ...
        assert result["file_count"] == 9, (
            f"_get_file_info应该找到9个文件，实际找到{result['file_count']}个"
        )
        assert result["last_modified"] is not None
    ...
    def test_file_creation_verification(self):
        """验证文件确实被创建了"""
        # 直接检查创建的文件列表
        assert len(self.files_created) == 9, (
            f"应该创建9个文件，实际创建{len(self.files_created)}个"
        )
    ...
    def test_directory_structure(self):
        ...
        for year in years:
            ...
            assert len(files_in_year) == 3, (
                f"年份 {year} 应该有3个文件，实际有{len(files_in_year)}个"
            )
