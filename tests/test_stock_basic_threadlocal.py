...
from src.downloader2.models.stock_basic import StockBasicFetcher
...
class TestStockBasicThreadLocal:
    ...
    def setup_method(self):
        ...
        StockBasicFetcher.clear_instance()

    def test_single_thread_singleton(self):
        """测试单线程中的单例行为"""
        with patch("tushare.set_token"):
            instance1 = StockBasicFetcher.get_instance()
            instance2 = StockBasicFetcher.get_instance()
    ...
    def test_different_threads_different_instances(self):
        ...
        def create_instance(thread_id: int):
            with patch("tushare.set_token"):
                instance = StockBasicFetcher.get_instance()
    ...
    def test_clear_instance_different_threads(self):
        ...
        def thread1_operations():
            with patch("tushare.set_token"):
                instances["thread1_before"] = StockBasicFetcher.get_instance()
                time.sleep(0.1)  # 等待thread2创建实例
                StockBasicFetcher.clear_instance()  # 只清除当前线程的实例
                instances["thread1_after"] = StockBasicFetcher.get_instance()

        def thread2_operations():
            with patch("tushare.set_token"):
                instances["thread2_before"] = StockBasicFetcher.get_instance()
                time.sleep(0.2)  # 等待thread1清除实例
                instances["thread2_after"] = StockBasicFetcher.get_instance()
    ...
    def test_constructor_still_works(self):
        ...
        with patch("tushare.set_token"):
            # 直接构造
            direct_instance = StockBasicFetcher()

            # 通过单例获取
            singleton_instance = StockBasicFetcher.get_instance()
