...
class TestStockBasicThreadLocal:
    ...
    def test_single_thread_singleton(self):
        ...
        with patch('tushare.set_token'):
            instance1 = StockBasic.get_instance()
            instance2 = StockBasic.get_instance()
    ...
    def test_different_threads_different_instances(self):
        ...
        def create_instance(thread_id: int):
            with patch('tushare.set_token'):
                instance = StockBasic.get_instance()
    ...
    def test_clear_instance_different_threads(self):
        ...
        def thread1_operations():
            with patch('tushare.set_token'):
                instances['thread1_before'] = StockBasic.get_instance()
                time.sleep(0.1)  # 等待thread2创建实例
                StockBasic.clear_instance()  # 只清除当前线程的实例
                instances['thread1_after'] = StockBasic.get_instance()
        
        def thread2_operations():
            with patch('tushare.set_token'):
                instances['thread2_before'] = StockBasic.get_instance()
                time.sleep(0.2)  # 等待thread1清除实例
                instances['thread2_after'] = StockBasic.get_instance()
    ...
    def test_constructor_still_works(self):
        ...
        with patch('tushare.set_token'):
            # 直接构造
            direct_instance = StockBasic()
            
            # 通过单例获取
            singleton_instance = StockBasic.get_instance()
