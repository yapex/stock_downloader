...
class MockFactory:
    ...
    def create_db_queryer_mock(
        latest_trading_day: str = "20240115", max_dates: Optional[Dict[str, str]] = None
    ) -> Mock:
    ...
    def create_complete_rate_limit_mocks(
        cls, download_tasks: Optional[Dict[str, Dict]] = None
    ) -> Dict[str, Any]:
        ...
        return {"config": config_mock}

    @staticmethod
    def create_tushare_api_mock(
        token: str = "test_token", api_functions: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Mock]:
        ...
        return {
            ...
            "pro": pro_mock,
            "api_functions": {
                name: getattr(pro_mock, name) for name in api_functions.keys()
            },
        }
    ...
    @staticmethod
    def create_normalize_stock_code_mock(normalized_code: str = "600519.SH") -> Mock:
    ...
    def create_complete_fetcher_builder_mocks(
        ...
        return {**tushare_mocks, "normalize_stock_code": normalize_mock}

    @staticmethod
    def create_mock_api_manager(api_function_return_value=None) -> Mock:
        """创建一个模拟的 API 管理器
        ...
        if api_function_return_value is None:
            api_function_return_value = Mock(
                return_value=pd.DataFrame({"data": [1, 2, 3]})
            )
...
class CliRunnerWrapper:
    ...
    def invoke_dp_command(self, queue_name: str, debug: bool = False):
        """调用 dp 命令的辅助方法
