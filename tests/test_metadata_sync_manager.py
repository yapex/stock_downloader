...
class TestMetadataSyncManager:
    ...
    def test_init(self, temp_directories):
        ...
        assert (
            manager.cache_path.exists() or not manager.cache_path.exists()
        )  # 缓存文件可能存在也可能不存在
    ...
    def test_quick_table_check_file_count_change(self, sample_parquet_structure):
        ...
        # 再次检查，文件数量未变
        manager._quick_table_check("stock_adj_hfq", minutes=30)
        # 这里应该根据时间窗口判断

        # 添加新文件
        new_file = (
            Path(sample_parquet_structure["parquet_path"])
            / "stock_adj_hfq"
            / "year=2025"
            / "new_file.parquet"
        )
    ...
    def test_integration_sync_with_real_db(self, sample_parquet_structure):
        ...
        with duckdb.connect(sample_parquet_structure["db_path"]) as con:
            ...
            try:
                ...
                con.execute("SELECT COUNT(*) FROM stock_adj_hfq").fetchone()
                # 如果能执行到这里说明视图创建成功
    ...
    def test_sync_skip_logic(self, sample_parquet_structure):
        ...
        with patch("builtins.print") as mock_print:
            manager.sync(
                force_full_scan=False, mtime_check_minutes=60
            )  # 只检查最近1小时的文件
    ...
    def test_error_handling(self, temp_directories):
        ...
        manager = MetadataSyncManager(
            temp_directories["db_path"], str(nonexistent_path)
        )
...
class TestSpecificBugScenarios:
    ...
    def test_missing_stock_scenario(self, temp_directories):
        ...
        for stock in test_stocks:
            for year in ["2023", "2024"]:
                ...
                # 创建包含股票代码的文件
                parquet_file = (
                    year_dir / f"part-{stock.replace('.', '')}-{year}.parquet"
                )
                df = pd.DataFrame(
                    {
                        "ts_code": [stock],
        ...
        with duckdb.connect(temp_directories["db_path"]) as con:
            views = con.execute(
                "SELECT view_name FROM duckdb_views() WHERE view_name = 'stock_adj_hfq'"
            ).fetchall()
            assert len(views) == 1
    ...
    def test_partial_sync_detection(self, sample_parquet_structure):
        ...
        df = pd.DataFrame(
            {"ts_code": ["999999.SZ"], "trade_date": ["2026-01-01"], "close": [500.0]}
        )
        table = pa.Table.from_pandas(df)
