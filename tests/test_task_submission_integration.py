...
class TestTaskSubmissionIntegration:
    ...
    def test_task_submission_to_queue(
        self,
        mock_fetcher_builder_class,
        mock_process_task,
        huey_immediate,
        setup_test_database,
    ):
    ...
    def test_multiple_task_submissions(
        ...
        # 验证至少有一些任务成功下载
        assert successful_downloads > 0, "没有任务成功下载"

        # 验证任务被提交到队列的次数等于成功下载的次数
        assert mock_process_task.call_count == successful_downloads, (
            f"期望 {successful_downloads} 次任务提交，实际 {mock_process_task.call_count} 次"
        )

    @patch("neo.task_bus.huey_task_bus.process_task_result")
    @patch("neo.downloader.simple_downloader.SimpleDownloader._fetch_data")
    def test_task_submission_with_mock_data(
        self, mock_fetch_data, mock_process_task, huey_immediate, setup_test_database
    ):
        """使用模拟数据测试任务提交"""
        import pandas as pd
    ...
    @patch("neo.task_bus.huey_task_bus.process_task_result")
    @patch("neo.downloader.simple_downloader.FetcherBuilder")
    def test_queue_task_processing_status(
        self,
        mock_fetcher_builder_class,
        mock_process_task,
        huey_immediate,
    ...
    @patch("neo.task_bus.huey_task_bus.process_task_result")
    @patch("neo.downloader.simple_downloader.FetcherBuilder")
    def test_huey_instance_availability(
        self,
        mock_fetcher_builder_class,
        mock_process_task,
        huey_immediate,
