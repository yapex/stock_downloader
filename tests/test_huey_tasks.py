...
class MockContainer:
    ...
    def __init__(self):
        ...
        self._downloader_instance.download.return_value = pd.DataFrame({"ts_code": ["000001.SZ"]})
        ...
        self._group_handler_instance.get_task_types_for_group.return_value = ["stock_daily"]
        self._group_handler_instance.get_symbols_for_group.return_value = ["000001.SZ", "000002.SZ"]
...
class TestDownloadTask:
    ...
    def test_download_task_success_and_chains(self, mock_process_task, mock_container):
        ...
        mock_downloader_instance.download.return_value = pd.DataFrame({"test": [1, 2, 3]})
    ...
    @patch("neo.app.container")
    @patch("neo.tasks.data_processing_tasks.process_data_task")
    def test_download_task_empty_data_does_not_chain(self, mock_process_task, mock_container):
    ...
    @patch("neo.app.container")
    @patch("neo.tasks.data_processing_tasks.process_data_task")
    def test_download_task_none_data_does_not_chain(self, mock_process_task, mock_container):
    ...
    def test_download_task_exception_handling(self, mock_logger, mock_process_task, mock_container):
        """测试下载任务执行时抛出异常"""
        from neo.tasks.huey_tasks import download_task
...
class TestProcessDataTask:
    ...
    def test_process_data_task_with_data(self, mock_data_processor_class):
        ...
        mock_processor_instance.process_data.assert_called_once_with(TaskType.stock_basic, "000001.SZ", data)
    ...
    def test_process_data_task_with_no_data(self, mock_data_processor_class):
        ...
        mock_data_processor_class.assert_called_once()
        mock_processor_instance.process_data.assert_called_once_with(TaskType.stock_basic, "000001.SZ", [])
    ...
    @patch("neo.tasks.data_processing_tasks.DataProcessor")
    def test_process_data_task_exception_handling(self, mock_data_processor_class, mock_logger):
        """测试异常处理"""
        from neo.tasks.data_processing_tasks import process_data_task
        ...
        # 设置mock实例抛出异常
        mock_processor_instance = Mock()
        mock_processor_instance.process_data.side_effect = Exception("Processing failed")
        mock_data_processor_class.return_value = mock_processor_instance
...
class TestBuildAndEnqueueTask:
    ...
    def test_build_and_enqueue_logic(self, mock_container_func, mock_get_config, mock_parquet_operator, mock_schema_loader, mock_download_task):
        ...
        for attr_name in dir(mock_container):
            if not attr_name.startswith('_'):
                setattr(mock_container_func, attr_name, getattr(mock_container, attr_name))
        ...
        try:
            result = build_and_enqueue_downloads_task.func("all_stocks", stock_codes=None)
            print(f"执行结果: {result}")
    ...
    @patch("neo.tasks.download_tasks.get_config")
    @patch("neo.app.container")
    def test_build_and_enqueue_exception_handling(self, mock_container_func, mock_get_config, mock_download_task, mock_logger):
        """测试构建和派发任务的异常处理"""
        from neo.tasks.download_tasks import build_and_enqueue_downloads_task
        ...
        # 设置group_handler抛出异常
        mock_container._group_handler_instance.get_task_types_for_group.side_effect = Exception("构建下载任务失败")
