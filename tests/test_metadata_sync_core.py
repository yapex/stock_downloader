...
class TestMetadataSyncCoreLogic:
    ...
    def test_initialization(self, temp_directories):
        ...
        manager = MetadataSyncManager(
            temp_directories["db_path"], temp_directories["parquet_path"]
        )
    ...
    def test_physical_partition_detection(self, stock_hfq_structure):
        """测试物理分区检测 - 这是关键功能"""
        manager = MetadataSyncManager(
            stock_hfq_structure["db_path"], stock_hfq_structure["parquet_path"]
        )
    ...
    def test_quick_table_check_logic(self, stock_hfq_structure):
        ...
        # 模拟添加新文件
        new_file = (
            Path(stock_hfq_structure["parquet_path"])
            / "stock_adj_hfq"
            / "year=2025"
            / "new_part.parquet"
        )
        df = pd.DataFrame(
            {"ts_code": ["000999.SZ"], "trade_date": ["2025-01-01"], "close": [150.0]}
        )
        table = pa.Table.from_pandas(df)
    ...
    def test_cache_reliability(self, stock_hfq_structure):
        ...
        assert (
            manager2.cache["stock_adj_hfq"]["file_count"]
            == original_cache["stock_adj_hfq"]["file_count"]
        )
    ...
    def test_time_window_logic(self, stock_hfq_structure):
        ...
        df = pd.DataFrame(
            {"ts_code": ["000888.SZ"], "trade_date": ["2023-01-01"], "close": [80.0]}
        )
        table = pa.Table.from_pandas(df)
...
class TestSyncMissDetectionScenarios:
    ...
    def test_scenario_partial_file_update(self, stock_hfq_structure):
        ...
        assert (
            updated_scan["has_changes"] is True
            or updated_scan["last_modified"] > initial_scan["last_modified"]
        )
    ...
    def test_scenario_new_partition_added(self, stock_hfq_structure):
        ...
        # 添加新年份分区
        new_year_dir = (
            Path(stock_hfq_structure["parquet_path"]) / "stock_adj_hfq" / "year=2026"
        )
        new_year_dir.mkdir(exist_ok=True)
        new_file = new_year_dir / "part-0-2026.parquet"
        df = pd.DataFrame(
            {"ts_code": ["000777.SZ"], "trade_date": ["2026-01-01"], "close": [300.0]}
        )
        table = pa.Table.from_pandas(df)
        pq.write_table(table, new_file)
