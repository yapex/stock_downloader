...
class TestBatchProcessing:
    ...
    def test_mixed_task_types_batching(self):
        ...
        for task_type, api_method, data_dict in tasks:
            ...
            result = TaskResult(
                config=config, success=True, data=data, error=None
            )
    ...
    def test_get_stats_with_batch_info(self):
        """测试获取包含批量处理信息的统计数据"""
        # 添加一些数据到缓冲区
        test_data = pd.DataFrame({"ts_code": ["000001.SZ"], "symbol": ["000001"], "name": ["平安银行"]})
    ...
    def test_large_data_auto_flush(self):
        ...
        result = TaskResult(
            config=config, success=True, data=large_data, error=None
        )
    ...
    def test_flush_buffer_by_row_count(self):
        ...
        large_data = pd.DataFrame({
            "ts_code": [f"large{i}.SZ" for i in range(1, 11)],  # 10 行数据 > batch_size=3
    ...
    def test_multiple_dataframes_row_count_calculation(self):
        ...
        data1 = pd.DataFrame({"ts_code": ["001.SZ"], "symbol": ["001"], "name": ["股票1"]})
        data2 = pd.DataFrame({"ts_code": ["002.SZ"], "symbol": ["002"], "name": ["股票2"]})
        data3 = pd.DataFrame({"ts_code": ["003.SZ"], "symbol": ["003"], "name": ["股票3"]})
        data4 = pd.DataFrame({"ts_code": ["004.SZ"], "symbol": ["004"], "name": ["股票4"]})
