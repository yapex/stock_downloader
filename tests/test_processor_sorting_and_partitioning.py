...
def test_processor_sorts_and_partitions_correctly_for_incremental(
    ...
    expected_sorted_data = unsorted_data.sort_values(by=["ts_code", "ann_date"], ignore_index=True)
    ...
    processor = SimpleDataProcessor.create_default(
        ...
        schema_loader=real_schema_loader
    )
    ...
    assert passed_partition_cols == ["year"], f"分区列不正确，期望 ['year']，实际 {passed_partition_cols}"
