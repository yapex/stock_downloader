...
import pandas as pd
from neo.helpers.task_builder import TaskType
...
class TestDataProcessor:
    ...
    def test_validate_data_frame_not_list(self):
        """测试非列表数据验证"""
        with pytest.raises(ValueError, match="数据为空或格式无效"):
            self.processor._validate_data_frame("invalid_data", "stock_basic", "000001.SZ")
...
class TestProcessDataSync:
    ...
    def test_process_data_sync_success(self):
        ...
        # Mock DataProcessor
        with patch("neo.tasks.data_processing_tasks.DataProcessor") as mock_processor_class:
            mock_processor = Mock()
...
class TestProcessDataTaskIntegration:
    ...
    def test_process_data_task_success_with_logging(self, mock_logger):
        ...
        with patch("neo.tasks.data_processing_tasks.DataProcessor") as mock_processor_class:
            ...
            assert result is True
            mock_processor_class.assert_called_once()
            mock_processor.process_data.assert_called_once_with("stock_basic", "000001.SZ", data)
