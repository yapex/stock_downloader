...
class TestGroupHandler:
    ...
    def test_get_symbols_for_group_test(self, mock_get_config):
        """测试获取 test 组的股票代码"""
        # 模拟配置
        mock_config = Box({"task_groups": {"test": ["stock_daily"]}})
    ...
    def test_get_symbols_for_group_stock_basic(
        self, mock_db_operator_class, mock_get_config
    ):
        """测试获取 stock_basic 组的股票代码（从数据库）"""
        # 模拟配置
        mock_config = Box({"task_groups": {"stock_basic": ["stock_basic"]}})
        mock_get_config.return_value = mock_config
    ...
    @patch("neo.helpers.group_handler.get_config")
    @patch("neo.database.operator.DBOperator")
    def test_get_symbols_for_group_stock_basic_no_db_operator(
        self, mock_db_operator_class, mock_get_config
    ):
    ...
    def test_get_task_types_for_group_multiple_types(self, mock_get_config):
        """测试获取包含多个任务类型的组"""
        # 模拟配置
        mock_config = Box({"task_groups": {"all": ["stock_basic", "stock_daily"]}})
        mock_get_config.return_value = mock_config
    ...
    def test_get_task_types_for_group_invalid_task_type(self, mock_get_config):
        """测试无效任务类型的处理"""
        # 模拟配置
        mock_config = Box({"task_groups": {"invalid": ["invalid_task_type"]}})
        mock_get_config.return_value = mock_config
