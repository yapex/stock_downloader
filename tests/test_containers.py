...
class TestAppContainer:
    ...
    def test_data_processor_initialization_should_work_after_fix(self):
        ...
        try:
            ...
            print(
                f"✅ schema_loader 也正常注入：{type(data_processor.schema_loader).__name__}"
            )
        except TypeError as e:
            if "missing 1 required positional argument: 'schema_loader'" in str(e):
                pytest.fail(
                    f"❌ data_processor 仍然缺少 schema_loader 参数，容器配置需要修复：{e}"
                )
            else:
                ...
                assert (
                    "missing 1 required positional argument: 'schema_loader'"
                    not in str(e)
                )

    @patch("neo.configs.get_config")
    def test_data_processor_initialization_should_work_with_schema_loader(
        ...
        try:
            ...
            if "missing 1 required positional argument: 'schema_loader'" in str(e):
                pytest.fail(
                    "data_processor 仍然缺少 schema_loader 参数，容器配置需要修复"
                )
            else:
                # 其他 TypeError 可能是预期的（比如文件不存在等）
                pass
        ...
        for component_name, container_attr in components_to_test:
            try:
                ...
                if "missing" in str(e) and "required positional argument" in str(e):
                    ...
                    print(
                        f"⚠️  {component_name} 初始化时遇到其他错误（可能是正常的）: {e}"
                    )
