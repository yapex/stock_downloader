...
class TestStockBasicFieldsFromSeries:
    ...
    def test_from_series_with_complete_data(self):
        ...
        assert result.en_name == "Ping An Bank Co., Ltd."
    ...
    def test_from_series_missing_required_fields(self):
        ...
        assert result.name == "平安银行"
        # 验证缺少的字段不存在
        assert not hasattr(result, "ts_code")
    ...
    def test_from_series_with_empty_series(self):
        ...
        assert isinstance(result, StockBasicFields)
        # 验证没有任何字段属性
        class_attrs = vars(StockBasicFields)
        fields = [attr for attr in class_attrs if attr.isupper()]
        for field in fields:
            assert not hasattr(result, field.lower())

    def test_from_series_field_mapping(self):
        """测试字段映射是否正确"""
        # 验证所有字段常量都能正确映射
        data = {
            StockBasicFields.TS_CODE: "test_ts_code",
            StockBasicFields.SYMBOL: "test_symbol",
            StockBasicFields.NAME: "test_name",
            StockBasicFields.AREA: "test_area",
            StockBasicFields.INDUSTRY: "test_industry",
            StockBasicFields.FULL_NAME: "test_fullname",
            StockBasicFields.EN_NAME: "test_enname",
            StockBasicFields.CNSPELL: "test_cnspell",
            StockBasicFields.MARKET: "test_market",
            StockBasicFields.EXCHANGE: "test_exchange",
            StockBasicFields.CURR_TYPE: "test_curr_type",
            StockBasicFields.LIST_STATUS: "test_list_status",
            StockBasicFields.LIST_DATE: "test_list_date",
            StockBasicFields.DELIST_DATE: "test_delist_date",
            StockBasicFields.IS_HS: "test_is_hs",
            StockBasicFields.ACT_NAME: "test_act_name",
            StockBasicFields.ACT_ENT_TYPE: "test_act_ent_type",
        }
        series = pd.Series(data)
        ...
        assert result.full_name == "test_fullname"
        assert result.en_name == "test_enname"
