...
class TestAsyncSimpleDataProcessor:
    ...
    async def test_process_with_batch_mode(
        self, mock_db_operator, mock_schema_loader, mock_data_buffer, sample_data
    ):
        """测试批量处理模式"""
    ...
    async def test_process_empty_data(
        self, mock_db_operator, mock_schema_loader, mock_data_buffer
    ):
        """测试处理空数据"""
        processor = AsyncSimpleDataProcessor(
            db_operator=mock_db_operator,
    ...
    @pytest.mark.asyncio
    async def test_shutdown_without_data_buffer(
    ...
    @pytest.mark.asyncio
    async def test_data_buffer_initialization(
        ...
        with patch(
            "src.neo.data_processor.data_buffer.get_async_data_buffer"
        ) as mock_get_buffer:
