...
def mock_daily_df_part1():
    """提供第一批模拟的日线数据。"""
    return pd.DataFrame({'ts_code': ['600519.SH']*3, 'trade_date': ['20231009', '20231010', '20231011'],
                         'open': [1800.0, 1810.0, 1820.0], 'close': [1805.0, 1815.0, 1825.0]})

@pytest.fixture
def mock_daily_df_part2():
    """提供第二批（增量）模拟日线数据。"""
    return pd.DataFrame({'ts_code': ['600519.SH']*3, 'trade_date': ['20231011', '20231012', '20231013'],
                         'open': [1821.0, 1830.0, 1840.0], 'close': [1826.0, 1835.0, 1845.0]})
...
def test_storage_initial_save_and_get_date(temp_storage, mock_daily_df_part1):
    ...
    pd.testing.assert_frame_equal(df.sort_values(by='trade_date').reset_index(drop=True), saved_df)

    latest_date = storage.get_latest_date(data_type, ts_code, date_col='trade_date')
    assert latest_date == '20231011'

def test_storage_incremental_save(temp_storage, mock_daily_df_part1, mock_daily_df_part2):
    """测试增量更新功能，包括去重。"""
    storage = temp_storage
    ts_code = "600519.SH"
    data_type = "daily_qfq"
