...
from neo.main import app, dl, dp, main
...
class TestDlCommand:
    ...
    def test_dl_with_symbols_parameter(self, mock_task, mock_container, mock_logging):
        ...
        # 验证结果
        assert result.exit_code == 0
        mock_task.assert_called_once_with(group_name='test_group', stock_codes=['000001.SZ', '000002.SZ'])
...
class TestDpCommand:
    ...
    def test_dp_app_service_exception_handling(self, mock_container, mock_logging):
        ...
        mock_app_service = Mock()
        mock_app_service.run_data_processor.side_effect = Exception("数据处理器启动失败")
        mock_container.app_service.return_value = mock_app_service
...
class TestEdgeCases:
    ...
    def test_dp_with_invalid_queue_name(self):
        ...
        with patch('neo.helpers.utils.setup_logging') as mock_logging, \
