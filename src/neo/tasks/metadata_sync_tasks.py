...
class MetadataSyncManager:
    ...
    def _get_project_paths(self) -> tuple[Path, Path, Path]:
        ...
        if self._project_root is None:
            ...
            self._parquet_base_path = self._project_root / self.config.storage.parquet_base_path
            self._metadata_db_path = self._project_root / self.config.database.metadata_path
            
            logger.info(f"诊断: 项目根目录: {self._project_root}")
            logger.info(f"诊断: Parquet 根目录: {self._parquet_base_path}")
            logger.info(f"诊断: 元数据DB路径: {self._metadata_db_path}")
    ...
    def _setup_duckdb_connection(self, metadata_db_path: Path) -> duckdb.DuckDBPyConnection:
        """设置 DuckDB 连接和内存限制
    ...
    def _scan_parquet_directories(self, parquet_base_path: Path) -> List[Path]:
        ...
        logger.info(f"诊断: 找到 {len(table_dirs)} 个表目录")
        
        return table_dirs
    
    def _drop_existing_table_or_view(self, con: duckdb.DuckDBPyConnection, table_name: str) -> None:
        """删除已存在的表或视图
    ...
    def _create_metadata_view(self, con: duckdb.DuckDBPyConnection, table_name: str, table_glob_path: str) -> None:
        """创建元数据视图
        
        Args:
            con: DuckDB 连接
            table_name: 表名
            table_glob_path: Parquet 文件路径模式
        """
    ...
    def _sync_table_metadata(self, con: duckdb.DuckDBPyConnection, table_dir: Path) -> None:
        """同步单个表的元数据
        ...
        # 创建新的视图
        self._create_metadata_view(con, table_name, table_glob_path)
    ...
    def sync_metadata(self) -> None:
        ...
        try:
            # 1. 获取项目路径
            project_root, parquet_base_path, metadata_db_path = self._get_project_paths()
