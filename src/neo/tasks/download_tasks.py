...
class DownloadTaskManager:
    ...
    def _get_task_types_and_symbols(self, group_name: str, stock_codes: Optional[List[str]]) -> Tuple[List[str], Optional[List[str]]]:
        """获取任务类型和股票代码列表
        ...
        if stock_codes:
            ...
            logger.info(f"使用任务组 '{group_name}' 配置的股票代码: {len(symbols) if symbols else 0} 个")
        
        return task_types, symbols
    
    def _query_max_dates(self, task_types: List[str], symbols: Optional[List[str]]) -> Dict[Tuple[str, str], Optional[str]]:
    ...
    def _enqueue_download_tasks(self, task_types: List[str], symbols: Optional[List[str]], max_dates: Dict[Tuple[str, str], Optional[str]]) -> int:
        """派发下载任务到快速队列
        ...
        for task_type in task_types:
            if symbols:  # 有具体股票代码的任务类型
                for symbol in symbols:
                    ...
                    # 派发任务到快速队列
                    download_task(task_type=task_type, symbol=symbol, start_date=start_date)
                    enqueued_count += 1
            else:  # stock_basic 等不需要具体股票代码的任务类型
...
@huey_slow.task()
def build_and_enqueue_downloads_task(group_name: str, stock_codes: Optional[List[str]] = None):
    """构建并派发增量下载任务 (慢速队列)
    ...
    try:
        ...
        # 1. 获取任务类型和股票代码
        task_types, symbols = task_manager._get_task_types_and_symbols(group_name, stock_codes)
        if not task_types:
            return
        ...
        # 3. 派发下载任务
        enqueued_count = task_manager._enqueue_download_tasks(task_types, symbols, max_dates)
