...
class DataProcessor:
    ...
    def _validate_data_frame(self, data_frame: List[Dict[str, Any]], task_type: str, symbol: str) -> pd.DataFrame:
        """验证并转换数据格式
        ...
        try:
            ...
            logger.debug(f"🐌 [HUEY_SLOW] 数据验证通过: {symbol}_{task_type}, 数据行数: {len(df_data)}")
            return df_data
    ...
    def process_data(self, task_type: str, symbol: str, data_frame: List[Dict[str, Any]]) -> bool:
        """处理数据的主要方法
        ...
        try:
            ...
            # 处理数据
            logger.debug(f"🐌 [HUEY_SLOW] 开始异步保存数据: {symbol}_{task_type}, 数据行数: {len(df_data)}")
            ...
            logger.error(f"❌ [HUEY_SLOW] 数据处理异常: {symbol}_{task_type}, 错误: {e}", exc_info=True)
            raise e
...
@huey_slow.task()
def process_data_task(task_type: str, symbol: str, data_frame: List[Dict[str, Any]]) -> bool:
    """数据处理任务 (慢速队列)
