...
class CallbackQueueBuffer(IDataBuffer):
    ...
    def register_type(
        self,
        data_type: str,
        callback: Callable[[str, pd.DataFrame], bool],
        max_size: int = 100,
    ) -> None:
        """注册数据类型和对应的回调函数
    ...
    def get_buffer_sizes(self) -> Dict[str, int]:
        ...
        with self._buffer_lock:
            return {
                data_type: sum(len(df) for df in buffer)
    ...
    def _start_flush_thread(self) -> None:
        ...
        self._flush_thread = threading.Thread(
            target=self._timed_flush_loop, daemon=True
        )
...
class AsyncCallbackQueueBuffer:
    ...
    async def get_instance(
        cls, flush_interval: float = 30.0
    ) -> "AsyncCallbackQueueBuffer":
        """获取异步单例实例
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(callback):
                ...
                result = await loop.run_in_executor(
                    None, callback, data_type, combined_df
                )
...
def get_data_buffer(flush_interval: float = 30.0) -> AsyncCallbackQueueBuffer:
    ...
    try:
        ...
        if AsyncCallbackQueueBuffer._instance is None:
            AsyncCallbackQueueBuffer._instance = AsyncCallbackQueueBuffer(
                flush_interval
            )
...
async def get_async_data_buffer(
