...
class SimpleDataProcessor(IDataProcessor):
    ...
    def process(self, task_result: TaskResult) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.info(
                    f"数据处理完成: {task_result.config.task_type.value}, symbol: {task_result.config.symbol}, rows: {len(transformed_data)}"
                )
                ...
                logger.warning(
                    f"数据保存失败: {task_result.config.task_type}, symbol: {task_result.config.symbol}"
                )
                self.stats["failed_processed"] += 1
    ...
    def _output_stats(self) -> None:
        ...
        # 同时记录到日志
        logger.info(
            f"统计信息 - 总任务: {self.stats['total_processed']}, 成功: {self.stats['successful_processed']}, 失败: {self.stats['failed_processed']}, 成功率: {success_rate:.1f}%, 处理速率: {processing_rate:.2f} 任务/秒, 总行数: {self.stats['total_rows_processed']}"
        )
