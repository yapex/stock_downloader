...
class IAppService(Protocol):
    ...
    def run_downloader(self, tasks: List[DownloadTaskConfig], dry_run: bool = False) -> None:
        """运行下载器
        ...
        if dry_run:
            ...
            for task in tasks:
                task_name = f"{task.symbol}_{task.task_type.name}" if task.symbol else task.task_type.name
...
class AppService:
    ...
    def __init__(
        ...
        self._task_bus = task_bus or HueyTaskBus(self._data_processor)
    ...
    def run_producer_consumer(self, tasks: List[DownloadTaskConfig]) -> None:
        ...
        # 将任务添加到任务总线
        for task in tasks:
            self._task_bus.add_task(task)
        
        # 启动下载器（消费者）
        self._downloader.start_consumer(self._task_bus)
    ...
    def run_demo(self) -> None:
        ...
        print("2. 配置信息:")
        print(f"  数据库路径: {self._config.database.path}")
        print(f"  Tushare Token: {'已配置' if self._config.tushare.token else '未配置'}")
