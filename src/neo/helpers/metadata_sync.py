...
class MetadataSyncManager:
    ...
    def _load_schema_config(self) -> Dict[str, Any]:
        """从项目根目录自动查找并加载 stock_schema.toml"""
        # 从 self.parquet_path (e.g., /path/to/project/data/parquet) 开始向上查找
        current_dir = self.parquet_path
        project_root = current_dir
        # 循环直到找到包含 'stock_schema.toml' 的目录或到达文件系统根目录
        ...
        if not schema_file.exists():
            # 如果找不到，打印警告并返回空字典，而不是抛出错误
            print("警告: 未能自动定位 stock_schema.toml 文件，视图将不包含排序。")
            return {}
        ...
        try:
            ...
            print(
                f"警告: 加载或解析 stock_schema.toml 文件失败: {e}，视图将不包含排序。"
            )
            return {}
    ...
    def _quick_table_check(self, table_name: str, minutes: int) -> Dict[str, any]:
        ...
        if cached_last_modified and scan_info["last_modified"]:
            ...
            if (
                abs(scan_info["last_modified"] - cached_last_modified) > 1.0
            ):  # 允许1秒误差
                return {
                    "has_changes": True,
    ...
    def _fast_file_scan(
        self, table_name: str, minutes: int, dir_mtime: float = None
    ) -> Dict[str, any]:
        """快速文件扫描，优化后的版本。"""
        table_dir = self.parquet_path / table_name
        ...
        try:
            with duckdb.connect(database=str(self.db_path)) as con:
                for table_name in table_names:
                    # 获取排序配置
                    table_schema = self.schema_config.get(table_name, {})
                    primary_key = table_schema.get("primary_key")
                    order_by_clause = ""
                    if primary_key:
                        sort_columns = ", ".join(primary_key)
                        order_by_clause = f"ORDER BY {sort_columns}"
                    ...
                    if not force_full_scan and mtime_check_minutes > 0:
                        if not file_info["has_changes"]:
                            ...
                            print(
                                f"  -> 表 '{table_name}' 无最近变动跳过 (文件数: {file_info['file_count']}个{time_info}){method_info}"
                            )
                            skipped_count += 1
                            continue
                    ...
                    if not physical_partitions:
                        ...
                        if any(table_dir.glob("*.parquet*")):
                            ...
                            sql = f"CREATE OR REPLACE VIEW {table_name} AS SELECT * FROM read_parquet('{glob_pattern}') {order_by_clause};"
                            con.execute(sql)

                            # 获取同步后的统计信息
                            after_stats = self._get_table_stats(con, table_name)
                            time_info = (
                                f", 最后修改: {self._format_time_diff(file_info['last_modified'])} "
                                if file_info["last_modified"]
                                else ""
                            )
                            row_change = (
                                after_stats["row_count"] - before_stats["row_count"]
                            )
                            row_change_str = (
                                f" (+{row_change})"
                                if row_change > 0
                            ...
                            print(
                                f"     ✓ 更新完成: {after_stats['row_count']:,} 行数据{row_change_str}, 未分区, {file_info['file_count']} 个文件{time_info}"
                            )
                            updated_count += 1
                        else:
                            print(
                    ...
                    # 2. 如果没有文件变化，但分区不一致或视图不存在
                        ...
                        physical_partitions != view_partitions
                        or not self._view_exists(con, table_name)
                    ):
                        needs_update = True
                    ...
                    if needs_update:
                        ...
                        print(
                            f"     ✓ 更新完成: {after_stats['row_count']:,} 行数据{row_change_str}, {after_stats['partition_count']} 个分区, {file_info['file_count']} 个文件{time_info}"
                        )
                        updated_count += 1
                    else:
                        # 显示跳过表的详细信息
                        ...
                        print(
                            f"  -> 表 '{table_name}' 状态最新跳过 ({before_stats['row_count']:,} 行, {before_stats['partition_count']} 分区, {file_info['file_count']} 文件{time_info})"
                        )
                        skipped_count += 1
