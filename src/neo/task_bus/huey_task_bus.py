...
class HueyTaskBus(ITaskBus):
    ...
    def submit_task(self, task_result: TaskResult) -> None:
        ...
        logger.debug(
            f"TaskResult已提交到队列: {task_result.config.task_type.value}, symbol: {task_result.config.symbol}, result: {result}"
        )
...
def _get_data_processor():
    ...
    这个函数负责创建数据处理器实例，保持任务函数与具体实现的解耦。
    在实际应用中，可以通过配置或依赖注入来选择不同的处理器实现。
    """
    # 延迟导入避免循环导入
...
def process_task_result(task_result_data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        config_data = task_result_data["config"]
        task_name = (
            f"{config_data['symbol']}_{config_data['task_type']}"
            if config_data["symbol"]
