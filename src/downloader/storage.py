...
class PartitionedStorage:
    ...
    def conn(self):
        ...
        if not hasattr(self._local, "connection"):
            ...
            logger.debug(
                f"为线程 {threading.current_thread().name} 创建新的分区表DuckDB连接"
            )
    ...
    def save_financial_data(self, df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            table_columns = [
                desc[0]
                for desc in self.conn.execute("DESCRIBE financial_data").fetchall()
            ]
    ...
    def query_daily_data_by_date_range(
        ...
        try:
            ...
            query = (
                f"SELECT * FROM daily_data {where_clause} ORDER BY ts_code, trade_date"
            )
            return self.conn.execute(query, params).df()
        except Exception as e:
    ...
    def overwrite(self, df: pd.DataFrame, data_type: str, entity_id: str):
        ...
        try:
            ...
            if data_type.startswith("stock_list") or data_type == "system":
                ...
                # 其他数据：删除该股票的所有数据
                self.conn.execute(
                    f"DELETE FROM {table_name} WHERE ts_code = ?", [entity_id]
                )
    ...
    def get_stock_list(self) -> pd.DataFrame:
        ...
        try:
            return self.conn.execute(
                "SELECT * FROM sys_stock_list ORDER BY ts_code"
            ).df()
        except Exception as e:
    ...
    def get_all_stock_codes(self) -> List[str]:
        ...
        try:
            ...
            daily_codes = self.conn.execute(
                "SELECT DISTINCT ts_code FROM daily_data"
            ).fetchall()
            financial_codes = self.conn.execute(
                "SELECT DISTINCT ts_code FROM financial_data"
            ).fetchall()
            fundamental_codes = self.conn.execute(
