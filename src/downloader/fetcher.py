...
class TushareFetcher(IFetcher):
    ...
    def fetch_daily_history(
        ...
        logger.info(
            f"[API调用] 获取日K线数据 - 股票: {ts_code}, 时间: {start_date}~{end_date}, 复权: {adjust}"
        )
        ...
        try:
            ...
            logger.info(
                f"[API调用成功] 日K线数据获取完成 - 股票: {ts_code}, 数据量: {data_count}条"
            )
        except Exception as e:
            logger.warning(
                f"[API调用失败] 日K线数据获取失败 - 股票: {ts_code}, 错误: {e}"
            )
            raise
    ...
    def fetch_daily_basic(
        ...
        ts_code = normalize_stock_code(ts_code)
        logger.info(
            f"[API调用] 获取每日指标 - 股票: {ts_code}, 时间: {start_date}~{end_date}"
        )
        ...
        try:
            ...
            logger.info(
                f"[API调用成功] 每日指标获取完成 - 股票: {ts_code}, 数据量: {data_count}条"
            )
            return df
        except Exception as e:
            logger.warning(
                f"[API调用失败] 每日指标获取失败 - 股票: {ts_code}, 错误: {e}"
            )
            raise
    ...
    def fetch_income(
        ...
        logger.info(
            f"[API调用] 获取利润表 - 股票: {normalized_code}, 时间: {start_date}~{end_date}"
        )
        ...
        try:
            ...
            logger.info(
                f"[API调用成功] 利润表获取完成 - 股票: {normalized_code}, 数据量: {data_count}条"
            )
            return df
        except Exception as e:
            logger.warning(
                f"[API调用失败] 利润表获取失败 - 股票: {normalized_code}, 错误: {e}"
            )
            raise
    ...
    def fetch_balancesheet(
        ...
        normalized_code = normalize_stock_code(ts_code)
        logger.info(
            f"[API调用] 获取资产负债表 - 股票: {normalized_code}, 时间: {start_date}~{end_date}"
        )
        ...
        try:
            ...
            logger.info(
                f"[API调用成功] 资产负债表获取完成 - 股票: {normalized_code}, 数据量: {data_count}条"
            )
            return df
        except Exception as e:
            logger.warning(
                f"[API调用失败] 资产负债表获取失败 - 股票: {normalized_code}, 错误: {e}"
            )
            raise
    ...
    def fetch_cashflow(
        ...
        normalized_code = normalize_stock_code(ts_code)
        logger.info(
            f"[API调用] 获取现金流量表 - 股票: {normalized_code}, 时间: {start_date}~{end_date}"
        )
        ...
        try:
            ...
            logger.info(
                f"[API调用成功] 现金流量表获取完成 - 股票: {normalized_code}, 数据量: {data_count}条"
            )
            return df
        except Exception as e:
            logger.warning(
                f"[API调用失败] 现金流量表获取失败 - 股票: {normalized_code}, 错误: {e}"
            )
            raise
