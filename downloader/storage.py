...
class ParquetStorage:
    ...
    def get_latest_date(
        self, data_type: str, ts_code: str, date_col: str = "trade_date"
    ) -> str | None:
        """获取本地存储的最新日期。"""
    ...
    def save(
        ...
        if date_col not in df.columns:
            logger.error(
                f"[{data_type}/{ts_code}] DataFrame 中缺少日期列 '{date_col}'，无法保存。"
            )
            return
        ...
        try:
            ...
            logger.info(
                f"[{data_type}/{ts_code}] 数据已成功保存/更新，总计 {len(combined_df)} 条。"
            )

        except Exception as e:
            logger.error(
                f"[{data_type}/{ts_code}] 保存到 Parquet 文件 {file_path} 时发生严重错误: {e}"
            )
