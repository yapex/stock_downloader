...
我们推荐的最终目录结构将遵循行业标准的 **Hive 分区模式**。这种模式不仅逻辑清晰，而且能被 DuckDB、Spark、Dask 等几乎所有现代数据处理引擎自动识别和优化。
...
1.  **第一层：业务类型 (Task Type)**
    -   **路径**: `data/parquet/<task_type>/`
    -   **示例**: `stock_daily/`, `balance_sheet/`
    -   **目的**: 这是最高级别的分类，直接对应我们的任务类型。它将不同数据模式（schema）的业务数据从物理上完全隔离。

2.  **第二层：日期分区 (Date Partition)**
    -   **路径**: `.../<partition_key>=<value>/`
    -   **示例**: `trade_date=2025-08-26/`, `end_date=2025-06-30/`
    -   **目的**: 这是**性能优化的核心**。当您执行一个带时间范围的查询时，DuckDB 会利用这个分区结构，只读取相关日期目录下的文件，这个过程被称为“**分区裁剪 (Partition Pruning)**”，可以指数级地提升查询性能。

3.  **第三层：数据文件 (Data Files)**
    -   **路径**: `.../part-0-....parquet`
    -   **关于 `symbol`**: 我们**刻意没有**在初始阶段按股票代码分区，以避免“**小文件问题**”（即海量小文件拖慢文件系统和查询引擎）。在每个日期分区内，单个 Parquet 文件包含了当天所有股票的数据，DuckDB 依然可以非常高效地在文件内部定位到特定股票。

##### 总结

我们采用 **“业务类型 -> 日期”** 的两层分区结构，它具有性能卓越、避免小文件问题、结构清晰和易于扩展的优势。
