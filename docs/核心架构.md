...
3.  **数据湖架构 (Data Lake Architecture)**: 项目采用现代数据湖架构，将数据存储与计算分离。
    ...
    - **分区存储**: 按 **`year` (年份)** 对数据进行分区，有效避免了因每日分区导致的海量“小文件问题”，显著提升了查询性能。
...
graph LR
    subgraph AppContainer [DI 容器]
        ...
        subgraph "消费者 (dp 命令)"
            ...
            ConsumerRunner --> SimpleDataProcessor["SimpleDataProcessor (统一处理器)"]
    ...
    process_data_task -- "5. 直接使用" --> SimpleDataProcessor
    SimpleDataProcessor -- "7. 写入" --> ParquetFiles
    huey_maint -- "8. 触发" --> sync_metadata
    sync_metadata -- "9. 扫描" --> ParquetFiles
    sync_metadata -- "10. 更新" --> MetadataDB
    ParquetDBQueryer -- "11. 查询" --> MetadataDB
```
...
-   **`SimpleDataProcessor` (Factory)**: **统一数据处理器**，根据 `config.toml` 中任务的 `update_strategy` 配置自动选择处理策略。支持增量更新（调用 `ParquetWriter.write`）和全量替换（调用 `ParquetWriter.write_full_replace`）两种模式。
-   **`ParquetWriter` (Factory)**: **Parquet 文件写入器**，提供两种写入方法：`write` (模拟追加) 和 `write_full_replace` (完全覆盖)。
...
-   **`process_data_task` (Huey Task)**: **数据处理任务 (慢速)**，在 `huey_slow` 中**单线程**执行。它直接使用 `SimpleDataProcessor`，处理器内部根据配置自动选择合适的写入策略。
...
3.  **消费慢速队列 (数据处理与写入)**:
    ...
    - **统一处理器**: 任务直接使用 `SimpleDataProcessor` 实例进行数据处理。
    - **策略选择**: `SimpleDataProcessor` 内部会读取 `config.toml` 中该任务对应的 `update_strategy` 配置，自动选择合适的写入策略。
    - **数据处理与写入**: 处理器根据配置策略调用 `ParquetWriter` 的不同方法将数据写入 Parquet 文件。
4.  **冲突处理与写入策略**:
    - **增量策略 (`update_strategy: incremental`)**: `SimpleDataProcessor` 调用 `ParquetWriter.write` 方法。此方法为了模拟"追加"数据到分区，会在生成文件名时加入**时间戳**。这确保了每次写入都是一个新文件，从而避免了覆盖整个分区的写操作。
        - **风险**: 此方法**不是幂等的**。如果对一个本应全量更新的表（如 `trade_cal`）重复执行此任务，会不断产生包含重复数据的新文件，导致数据冗余。
    - **全量替换策略 (`update_strategy: full_replace`)**: `SimpleDataProcessor` 调用 `ParquetWriter.write_full_replace` 方法，并通过**写入临时目录 -> 备份旧目录 -> 重命名 -> 删除备份**的原子性操作，安全地实现全量覆盖。
        - **优势**: 此方法是**幂等的**，可以安全地重复执行，非常适合 `stock_basic`、`trade_cal` 这类基础表的更新。
5.  **维护队列处理**:
...
2. **数据处理稳定性**:
   - 慢速队列单线程执行，避免文件写入竞争。
   - **策略化写入** (`full_replace` vs `incremental`) 保证了每种数据类型的一致性。
...
- **原瓶颈：文件并发写入与小文件问题**
  - **解决方案**:
    ...
    3.  引入**写入策略**区分增量追加和全量覆盖，解决了非幂等写入导致的重复数据问题。
