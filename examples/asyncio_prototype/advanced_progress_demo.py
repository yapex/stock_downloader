...
async def demo_nested_progress_bars():
    ...
    with tqdm(total=len(stock_groups), desc="📊 处理股票组", position=0) as group_pbar:
        for group_name, symbols in stock_groups.items():
            ...
            # 内层进度条：处理单个组内的股票
            with tqdm(
                total=len(symbols), desc=f"  📈 {group_name}", position=1, leave=False
            ) as stock_pbar:
                group_tasks = []
                ...
                for coro in asyncio.as_completed(
                    [aget_result(task) for task in group_tasks]
                ):
                    try:
                        result = await coro
...
async def demo_error_handling_with_progress():
    ...
    print("\n📊 错误处理统计:")
    print(
        f"  ✅ 成功: {success_count}/{len(tasks)} ({success_count / len(tasks) * 100:.1f}%)"
    )
    print(
        f"  ❌ 失败: {error_count}/{len(tasks)} ({error_count / len(tasks) * 100:.1f}%)"
    )
...
async def demo_pipeline_progress():
    ...
    with tqdm(total=3, desc="🔄 流水线进度", position=0) as pipeline_pbar:
        ...
        # 模拟下载进度监控
        with tqdm(
            total=100, desc="  📥 下载进度", position=1, leave=False
        ) as download_pbar:
            ...
            while True:
                ...
                try:
                    download_result = await asyncio.wait_for(
                        aget_result(download_task), timeout=0.1
                    )
                    download_pbar.n = 100
        ...
        with tqdm(
            total=100, desc="  📊 分析进度", position=1, leave=False
        ) as analysis_pbar:
            start_time = time.time()
            ...
            while True:
                ...
                try:
                    analysis_result = await asyncio.wait_for(
                        aget_result(analysis_task), timeout=0.1
                    )
                    analysis_pbar.n = 100
        ...
        with tqdm(
            total=100, desc="  📄 报告生成", position=1, leave=False
        ) as report_pbar:
            # 模拟报告生成过程
            report_steps = [
                "收集数据",
    ...
    print("\n📊 流水线处理结果:")
    print(
        f"  📥 下载结果: {download_result.get('success', 0)}/{download_result.get('total', 0)} 成功"
    )
    if analysis_result.get("status") == "completed":
        analysis = analysis_result.get("analysis", {})
        print(
            f"  📊 分析结果: 平均价格 ${analysis.get('avg_price', 0)}, 总成交量 {analysis.get('total_volume', 0):,}"
        )
    print("  📄 报告状态: 已生成")
